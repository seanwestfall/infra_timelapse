...
const ACCESS_ENV = {
  ...
  CLOUDFLARE_ACCESS_API_TOKEN: "access-policy-secret",
  REGISTRATION_ALLOWED_EMAIL: "person@example.com",
};
...
test("registers a validated email through the Access policy service", async () => {
  let received;
  const response = await handleRequest(
    new Request("https://infra.example/api/v1/register", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Origin: "https://infra.example",
  ...
  assert.equal(received, "person@example.com");
...
test("registration rejects invalid input and supports CORS preflight", async () => {
  const invalid = await handleRequest(
    new Request("https://infra.example/api/register", {
      method: "POST",
...
test("registration is restricted to the configured demo address", async () => {
  const response = await handleRequest(
    new Request("https://infra.example/api/v1/register", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
test("confirms a registration token through the confirmation service", async () => {
  let received;
  const token = "a".repeat(43);
  const response = await handleRequest(
    new Request("https://infra.example/api/v1/register/confirm", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
