...
import { addEmailToAccessPolicy, normalizeEmail } from "./access-policy.js";
import { confirmEmailToken, requestEmailConfirmation } from "./registration.js";
...
const CONFIRM_REGISTER_PATH = "/api/register/confirm";
...
function registrationCorsHeaders(corsOrigin) {
  return corsOrigin
    ? {
...
async function registrationResponse(request, env, corsOrigin, requestConfirmation) {
  const corsHeaders = registrationCorsHeaders(corsOrigin);
  ...
  if (!email) return jsonResponse({ error: "Enter a valid email address" }, 400, corsHeaders);
  const allowedEmail = normalizeEmail(env.REGISTRATION_ALLOWED_EMAIL);
  if (!allowedEmail || email !== allowedEmail) {
    return jsonResponse({ error: "Registration is not available for this address" }, 403, corsHeaders);
  }

  try {
    const result = await requestConfirmation(email, env);
    return jsonResponse(
      {
        pending: !result.alreadyConfirmed,
        message: result.alreadyConfirmed
          ? "This email address has already been confirmed."
          : "Check your inbox for a confirmation link.",
      },
      result.alreadyConfirmed ? 200 : 202,
      corsHeaders,
    );
  } catch (error) {
    console.error("Email confirmation request failed", error);
    return jsonResponse({ error: "Registration is temporarily unavailable" }, 502, corsHeaders);
  }
}

async function confirmationResponse(request, env, corsOrigin, confirmToken) {
  const corsHeaders = registrationCorsHeaders(corsOrigin);
  const contentType = request.headers.get("Content-Type") || "";
  if (!contentType.toLowerCase().startsWith("application/json")) {
    return jsonResponse({ error: "Content-Type must be application/json" }, 415, corsHeaders);
  ...
  try {
    const result = await confirmToken(body?.token, env);
    if (result.status === "invalid") {
      return jsonResponse({ error: "This confirmation link is invalid or expired" }, 400, corsHeaders);
    }
    return jsonResponse(
      { confirmed: true, message: "Your email is confirmed and access has been granted." },
      200,
      corsHeaders,
    );
  } catch (error) {
    console.error("Email confirmation failed", error);
    return jsonResponse({ error: "Confirmation is temporarily unavailable" }, 502, corsHeaders);
  }
}
...
export async function handleRequest(
  ...
  const canonicalPath = canonicalApiPath(incomingUrl.pathname);
  if ([REGISTER_PATH, CONFIRM_REGISTER_PATH].includes(canonicalPath)) {
    ...
    if (request.method === "OPTIONS") {
      return new Response(null, {
        status: 204,
        headers: {
          ...registrationCorsHeaders(cors.origin),
          "Cache-Control": "no-store",
        },
      });
    }
    ...
    if (canonicalPath === CONFIRM_REGISTER_PATH) {
      const confirmToken = dependencies.confirmEmailToken || ((token, runtimeEnv) =>
        confirmEmailToken(
          token,
          runtimeEnv,
          dependencies.addEmailToAccessPolicy || addEmailToAccessPolicy,
          { createDatabaseClient: dependencies.createDatabaseClient },
        ));
      return confirmationResponse(request, env, cors.origin, confirmToken);
    }
    const requestConfirmation = dependencies.requestEmailConfirmation || ((email, runtimeEnv) =>
      requestEmailConfirmation(email, runtimeEnv, {
        createDatabaseClient: dependencies.createDatabaseClient,
      }));
    return registrationResponse(request, env, cors.origin, requestConfirmation);
  }
